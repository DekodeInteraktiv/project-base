...
**Environment variables** — always read via the `\env()` helper (Symfony DotEnv), never `$_ENV`, `getenv()`, or `$_SERVER`:
```php
...
Always use the `~` (tilde) constraint when adding or updating dependencies in `composer.json` and `package.json`:
