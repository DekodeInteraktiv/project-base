...
function setup(): void {
	...
	\add_action( 'theme/hook/search-results-page-title', __NAMESPACE__ . '\\render_search_results_page_title' );
	\add_action( 'theme/hook/search-result-meta', __NAMESPACE__ . '\\render_search_result_meta' );
	\add_action( 'theme/hook/page-title', __NAMESPACE__ . '\\render_page_title' );
...
/**
 * Render the search results page title.
 */
function render_search_results_page_title(): void {
	printf(
		'<h1 class="search-page-title">%1$s</h1>',
		sprintf(
			/* Translators: %1$s is the search query keyword. */
			_x( 'Search results for %1$s', 'Title on the search page', 'dekode-starter-theme' ), // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			sprintf( '<mark data-search-keyword>%1$s</mark>', get_search_query() )
		)
	);
}
...
function render_wiki_page_title(): void {
	if ( \is_archive() ) {
		if ( \is_tax() ) {
			...
			printf(
				'<h1 class="wiki-page-title">%1$s
					<span class="description">%2$s</span>
				</h1>',
				\esc_html__( 'Knowledge Base', 'dekode-starter-theme' ),
				\esc_html__( 'Browse through all Knowledge Base articles.', 'dekode-starter-theme' )
			);
		}
	}
}
 ...
 */
function render_page_title(): void {
	if ( \is_home() ) {
		printf(
			'<h1>%1$s</h1>',
			\esc_html__( 'News', 'dekode-starter-theme' )
		);
	} elseif ( \is_archive() ) {
		global $wp_query;
		$post_type = \get_post_type_object( $wp_query->query['post_type'] ?? 'post' );
