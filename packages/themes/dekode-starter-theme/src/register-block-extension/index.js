...
function registerBlockExtension(
	 ...
	 * @return {boolean} true if the block is the one we want to add the extension to
	 */
	...
	const addSettingsToBlock = createHigherOrderComponent((BlockEdit) => {
		return (props) => {
			...
			const shouldRenderFallback =
				!shouldRenderBefore && !shouldRenderAfter && isSelected;
	 ...
	 */
	const addAdditionalPropertiesInEditor = createHigherOrderComponent(
		(BlockList) => {
			return (props) => {
				const { name, attributes, className, style, wrapperProps } =
					props;
